...
>(({ className, children, ...props }, ref) => {
  // Manage body scroll lock for mobile modal centering fix
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      // Add modal-open class to body when modal opens
      document.body.classList.add('modal-open')
      
      // Cleanup: remove modal-open class when modal closes
      return () => {
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          "fixed left-[50%] top-[50%] z-[101] grid translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]",
          // Mobile-first responsive classes with iOS Safari fixes
          "w-[95vw] max-w-[95vw] max-h-[95vh] p-4 rounded-lg",
          // iOS Safari specific fixes
          "supports-[height:100dvh]:max-h-[95dvh]",
          // Tablet and up
