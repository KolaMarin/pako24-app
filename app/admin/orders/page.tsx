...
import { Edit2, Save, X, Trash2, Package, Send, Download, Search } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  const updateProductDetails = async (orderId: string, productId: string, updatedProduct: Partial<ProductLink>) => {
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Sukses",
          description: "Detajet e produktit u përditësuan me sukses.",
        })
      } else {
        ...
        <CardContent className="pt-6">
          {isEditing ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium">Çmimi (GBP)</label>
                  <Input
                    type="number"
                    step="0.01"
                    value={editedProduct.priceGBP}
                    onChange={(e) => setEditedProduct({ ...editedProduct, priceGBP: Number(e.target.value) })}
                    className="mt-1"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditedProduct({ ...editedProduct, priceEUR: Number(e.target.value) })}
                    className="mt-1"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditedProduct({ ...editedProduct, customsFee: Number(e.target.value) })}
                    className="mt-1"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditedProduct({ ...editedProduct, transportFee: Number(e.target.value) })}
                    className="mt-1"
                  />
                </div>
              </div>
                    ...
                    <AccordionTrigger>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground">
                          (£{order.totalPriceGBP.toFixed(2)} / €{order.totalPriceEUR.toFixed(2)})
                        </span>
                      </div>
                    </AccordionTrigger>
                    <AccordionContent>
                          ...
                          <div className="mt-6 p-4 bg-muted/50 rounded-lg">
                            ...
                            <div className="space-y-2">
                              <div className="flex justify-between">
                                ...
                                <span>
                                  £{order.productLinks.reduce((sum, p) => sum + p.priceGBP * p.quantity, 0).toFixed(2)}
                                </span>
                              </div>
                              <div className="flex justify-between">
                              ...
                              <div className="pt-2 mt-2 border-t flex justify-between font-medium">
                                ...
                                <div className="text-right">
                                  <div>£{order.totalPriceGBP.toFixed(2)}</div>
